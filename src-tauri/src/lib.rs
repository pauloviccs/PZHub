...
use mod_manager::{
    ...
    open_steam_workshop, open_folder, delete_mod_folder, LocalModInfo, ModpackManifest,
};
...
#[tauri::command]
fn delete_local_mod_folder(folder_name: String) -> Result<(), String> {
    delete_mod_folder(&folder_name)
}
...
#[tauri::command]
fn launch_project_zomboid(ram_gb: u32) -> Result<String, String> {
    // Salva a preferência de RAM do usuário no UserConfig
    let mut cfg = load_config();
    cfg.allocated_ram_gb = if ram_gb == 0 { 8 } else { ram_gb };
