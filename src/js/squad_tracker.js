...
export class SquadTracker {
  ...
  updateSquad(squadState) {
    if (!squadState || !squadState.players) {
      ...
      this.updateMapMarkers();
      this.renderSidebarCards();
      if (this.onSquadUpdate) this.onSquadUpdate(this.players);
      return;
    }
    ...
    // Notifica ouvintes externos (ex: FriendsManager)
    if (this.onSquadUpdate) this.onSquadUpdate(this.players);
  ...
  /**
   * Alias defensivo para compatibilidade de chamadas de telemetria
   */
  updateTelemetry(squadState) {
    this.updateSquad(squadState);
  }
