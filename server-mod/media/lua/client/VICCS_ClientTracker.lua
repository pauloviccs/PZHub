...
function VICCS_Client.serializeJSON(players, serverName)
    ...
    for i, p in ipairs(players) do
        ...
        local pHealth = tonumber(p.health) or 100
        json = json .. comma .. '{'
        json = json .. '"id":"' .. escapeJSON(p.id or "p_unknown") .. '",'
        json = json .. '"name":"' .. escapeJSON(p.name or "Survivor") .. '",'
        json = json .. '"steam_id":"' .. escapeJSON(p.steam_id or "") .. '",'
        json = json .. '"x":' .. string.format("%.2f", px) .. ','
        json = json .. '"y":' .. string.format("%.2f", py) .. ','
        json = json .. '"z":' .. pz .. ','
        json = json .. '"health":' .. string.format("%.1f", pHealth) .. ','
        json = json .. '"faction":"' .. escapeJSON(p.faction or "Survivors") .. '",'
